...
class Nonce
{
    ...
    {
        if (sha1($this->request->getClientIp().$this->request->headers->get('User-Agent')) != substr(
    ...
    {
        if ($this->ttl !== null && is_int($this->ttl) && intval(
                substr(base64_decode($this->hash), 0, 10)
            ) + $this->ttl < time()
        ) {
    ...
    public function generate()
    {
        ...
        $this->hash = base64_encode(
            time().sha1($this->request->getClientIp().$this->request->headers->get('User-Agent')).bin2hex(
                random_bytes(32)
            )
        );
